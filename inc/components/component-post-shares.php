...
if (!function_exists('soren_post_shares')):
	...
	function soren_post_shares($togglecomment = false){
		...
		$out = true == $togglecomment && comments_open() || '0' != get_comments_number() ? sprintf('<section class="soren-post-shares">%s%s%s</section>',$twitter, $facebook, $togglecomment) : sprintf('<section class="soren-post-shares">%s%s</section>',$twitter, $facebook);
